...
@section('title', 'Tambah Pilar Kurikulum')

@section('content')
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Tambah Pilar Kurikulum</h1>
            <p class="text-xs text-slate-500 mt-0.5">Tambah kartu pilar metode pembelajaran baru.</p>
        </div>
    ...
    <div class="tailadmin-card p-6">
        <form action="{{ route('admin.curriculum.store') }}" method="POST" class="space-y-5">
            @csrf

            <div>
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">Judul Pilar *</label>
                <input type="text" name="title" value="{{ old('title') }}" required class="w-full text-sm py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-700" placeholder="Contoh: Digital Hybrid Learning">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full text-sm py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-700" placeholder="Jelaskan mengenai pilar pembelajaran ini...">{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="color" required class="w-full text-sm py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-700">
                        <option value="blue" {{ old('color') == 'blue' ? 'selected' : '' }}>Biru (Blue)</option>
                        <option value="purple" {{ old('color') == 'purple' ? 'selected' : '' }}>Ungu (Purple)</option>
                        <option value="emerald" {{ old('color') == 'emerald' ? 'selected' : '' }}>Hijau (Emerald)</option>
                        <option value="amber" {{ old('color') == 'amber' ? 'selected' : '' }}>Kuning/Oranye (Amber)</option>
                        <option value="rose" {{ old('color') == 'rose' ? 'selected' : '' }}>Merah (Rose)</option>
                        <option value="indigo" {{ old('color') == 'indigo' ? 'selected' : '' }}>Nila (Indigo)</option>
                    </select>
                </div>
            ...
            <div class="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" class="btn-primary">Simpan Pilar Kurikulum</button>
            </div>
        </form>
    </div>
</div>
