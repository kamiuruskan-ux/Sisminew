...
<div class="w-full space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center space-x-5">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-900 tracking-tight uppercase">Edit Pengumuman</h1>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 italic">{{ $announcement->title }}</p>
            </div>
        </div>
    ...
    <!-- Main Form -->
    <form id="announcementForm" action="{{ route('admin.announcements.update', encode_id($announcement->id)) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="type" class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:bg-white transition-all text-sm font-bold text-slate-900 appearance-none">
                                    <option value="general" {{ old('type', $announcement->type) == 'general' ? 'selected' : '' }}>Informasi Umum</option>
                                    <option value="academic" {{ old('type', $announcement->type) == 'academic' ? 'selected' : '' }}>Akademik</option>
                                    <option value="event" {{ old('type', $announcement->type) == 'event' ? 'selected' : '' }}>Kegiatan/Acara</option>
                                    <option value="urgent" {{ old('type', $announcement->type) == 'urgent' ? 'selected' : '' }}>Penting (Urgent)</option>
                                </select>
