...
<div class="space-y-6" x-data="{
    ...
    @include('components.delete-modal', ['title' => 'Hapus Jadwal Pelajaran', 'message' => 'Apakah Anda yakin ingin menghapus jadwal pelajaran :name ini? Tindakan ini tidak dapat dibatalkan.'])
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Kelola alokasi jadwal mata pelajaran, pengajar, ruangan, dan kelas secara terpusat.</p>
        </div>
        <div class="flex items-center space-x-3">
            <button type="button" onclick="openModal('createModal')" class="inline-flex items-center space-x-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-98 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-indigo-600/20">
    ...
    <div class="bg-white dark:bg-[#24303F] rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 shadow-2xs">
        <form method="GET" action="{{ route('admin.schedules.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 items-end">
            <!-- 1. Search Keyword -->
            <div>
                <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Cari Jadwal / Guru</label>
                <div class="relative">
                    <input type="text" 
                           name="search" 
                           value="{{ request('search') }}" 
                           placeholder="Mata pelajaran, guru, ruang..." 
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-2xs">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/60">
            <div>
                <h3 class="font-bold text-slate-900 text-sm">Daftar Sesi Jadwal Pelajaran</h3>
                <p class="text-xs text-slate-500">Menampilkan {{ $schedules->firstItem() ?? 0 }} - {{ $schedules->lastItem() ?? 0 }} dari {{ $schedules->total() }} total jadwal</p>
            </div>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
                <thead class="bg-slate-50/80 text-slate-500 uppercase tracking-wider font-bold text-[11px] border-b border-slate-100">
                    <tr>
                        <th class="px-6 py-3.5">Mata Pelajaran & Sesi</th>
                        <th class="px-6 py-3.5">Kelas Target</th>
                        <th class="px-6 py-3.5">Hari & Jam Pelaksanaan</th>
                        <th class="px-6 py-3.5">Guru Pengajar & Ruangan</th>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3.5">
                                    <div class="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 font-extrabold flex items-center justify-center text-xs border border-indigo-100 shrink-0 shadow-2xs">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                                        </svg>
                                    </div>
                                    <div>
                                        <p class="font-extrabold text-slate-900 text-xs">{{ $schedule->subject }}</p>
                                        <p class="text-[11px] text-slate-500 font-medium mt-0.5">{{ $schedule->name }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <!-- Class -->
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-3 py-1 bg-slate-100 text-slate-800 rounded-lg font-bold text-xs border border-slate-200/80">
                                    {{ $schedule->class->name ?? '-' }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                    <span class="inline-flex px-2.5 py-0.5 rounded-md bg-indigo-50 text-indigo-700 font-extrabold text-[10px] border border-indigo-200/80">
                                        {{ $days[$schedule->day] ?? $schedule->day }}
                                    </span>
                                    <p class="text-xs font-mono font-bold text-slate-900">
                            ...
                            <td class="px-6 py-4">
                                <div class="space-y-0.5">
                                    ...
                                    <p class="text-[11px] text-slate-500 font-medium flex items-center gap-1">
                                        ...
                                        <span>Ruang: <strong class="text-slate-700 font-mono">{{ $schedule->room }}</strong></span>
                                    </p>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($schedule->is_active)
                                    <span class="inline-flex px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-extrabold uppercase tracking-wider">
                                        Aktif
                                    </span>
                                @else
                                    <span class="inline-flex px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-extrabold uppercase tracking-wider">
                                        Nonaktif
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-1">
                                    <button type="button" onclick="openEditModal({{ json_encode($schedule) }})"
                                            class="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all" title="Edit Jadwal">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </button>

                                    <button type="button" @click="confirmDelete({{ $schedule->id }}, '{{ addslashes(($schedule->subject_name ?: 'Jadwal') . ' (' . ($schedule->class->name ?? '') . ')') }}')" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all cursor-pointer" title="Hapus Jadwal">
                                         <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
