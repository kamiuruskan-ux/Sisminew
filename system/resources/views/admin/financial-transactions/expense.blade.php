...
@section('title', 'Kas Keluar (Pengeluaran)')
@section('page_title', 'Pengeluaran Kas Sekolah')
...
<div class="space-y-6" x-data="{ 
    ...
    @include('components.delete-modal', ['title' => 'Hapus Transaksi Pengeluaran', 'message' => 'Apakah Anda yakin ingin menghapus transaksi pengeluaran :name ini? Saldo kas akan disesuaikan.'])


    <!-- Header Actions -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Pengeluaran Kas Sekolah</h1>
            <p class="text-xs text-slate-500 mt-1">Mencatat seluruh transaksi kas keluar (Gaji Guru, ATK, Listrik, Operasional, Maintenance, dll).</p>
        </div>
        <div>
            <button type="button" @click="createModalOpen = true" class="inline-flex items-center space-x-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold rounded-xl transition-colors shadow-xs">
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xs">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <h3 class="font-bold text-slate-900 text-sm">Riwayat Transaksi Pengeluaran Kas</h3>
            ...
            <table class="w-full text-left text-xs">
                <thead class="bg-slate-50 text-slate-500 uppercase tracking-wider font-semibold border-b border-slate-100">
                    <tr>
                        ...
                        <th class="px-5 py-3">Sumber Rekening</th>
                        <th class="px-5 py-3">Penerima Pembayaran</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($transactions as $tx)
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                <span class="px-2.5 py-0.5 rounded bg-rose-50 text-rose-700 border border-rose-200 font-bold text-xs">
                            ...
                            <td class="px-5 py-3.5 text-right font-bold text-rose-700 text-xs">Rp {{ number_format($tx->amount, 0, ',', '.') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center text-slate-400 text-xs font-medium">Belum ada transaksi pengeluaran kas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="createModalOpen" x-cloak class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 overflow-y-auto flex items-center justify-center p-4" @click="createModalOpen = false">
        <div class="bg-white rounded-xl shadow-xl max-w-lg w-full border border-slate-200 overflow-hidden" @click.stop>
            <div class="flex justify-between items-center px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                <h3 class="font-bold text-slate-900 text-sm">Catat Kas Keluar (Pengeluaran)</h3>
            ...
            <form action="{{ route('admin.financial-transactions.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Kategori Pengeluaran <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Kas / Bank Sumber <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Penerima Pembayaran</label>
                    <input type="text" name="recipient_or_payee" placeholder="Contoh: Toko ATK Sejahtera / PLN / PT Telekomunikasi" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs rounded-xl p-2.5 focus:bg-white focus:ring-1 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Lampiran Struk / Bukti Kwitansi (Opsional)</label>
                    <x-file-upload name="proof_file" accept=".jpg,.jpeg,.png,.pdf" label="Upload Struk / Kwitansi Pengeluaran" help="Seret & lepas foto/PDF kwitansi di sini" />
                </div>
