...
@section('content')
<div class="w-full space-y-6" x-data="{ slug: '{{ $role->slug }}' }">
    <!-- Header -->
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h2 class="text-lg font-bold text-gray-900">Edit Role</h2>
    ...
    <!-- Main Form -->
    <form id="roleForm" action="{{ route('admin.roles.update', encode_id($role->id)) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
            ...
            <div class="p-6">
                @if($role->isProtectedSystemRole())
                    <div class="p-4 bg-amber-50/90 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 text-amber-800 dark:text-amber-300 rounded-xl text-xs font-semibold flex items-center space-x-3 mb-6">
                        <svg class="w-5 h-5 text-amber-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                        </svg>
                        <span>Role ini adalah <strong>Role Sistem Bawaan</strong>. Nama Role dan Slug dikunci (tidak dapat diedit) untuk menjaga integritas sistem. Namun Anda tetap dapat mengelola Hak Akses (Permissions) dan Deskripsi.</span>
                    </div>
                @endif
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2 flex items-center justify-between">
                            <span>Nama Role <span class="text-red-500">*</span></span>
                            @if($role->isProtectedSystemRole())
                                <span class="text-[10px] bg-amber-100 text-amber-800 font-bold px-2 py-0.5 rounded-md flex items-center gap-1">
                        ...
                        <input type="text" 
                               ...
                               @if($role->isProtectedSystemRole())
                                   readonly
                                   class="w-full px-4 py-3 bg-gray-100 text-gray-600 border border-gray-300 rounded-xl cursor-not-allowed font-semibold focus:outline-none"
                               @else
                    ...
                    <div>
                        ...
                        <div class="flex">
                            ...
                            <input type="text" 
                                   ...
                                   @if($role->isProtectedSystemRole())
                                       readonly
                                       class="flex-1 px-4 py-3 bg-gray-100 text-gray-600 border border-gray-300 rounded-r-xl cursor-not-allowed font-mono text-sm focus:outline-none"
                                   @else
            ...
            <div class="p-6 space-y-6">
                @if($permissions && count($permissions) > 0)
                    ...
                    @if(isset($permissions['dashboard']))
                        @php
                            ...
                            $currentDashPerm = $dashPermissions->first(function($p) use ($rolePermIds) {
                                return in_array($p->id, $rolePermIds) || old('dashboard_permission') == $p->id;
                            });
                                    ...
                                    <label class="relative flex flex-col p-4 rounded-xl border transition-all cursor-pointer hover:border-indigo-300 group"
                                        ...
                                        <div class="flex items-start justify-between mb-2">
                                            ...
                                            <input type="radio" 
                                                   ...
                                                   {{ (in_array($permission->id, $rolePermIds) || old('dashboard_permission') == $permission->id) ? 'checked' : '' }}
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-sm text-gray-400 mt-1">Buat permission terlebih dahulu di menu Permissions</p>
                    </div>
                @endif
            </div>
        </div>
