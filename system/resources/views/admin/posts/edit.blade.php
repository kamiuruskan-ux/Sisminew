...
<div class="w-full space-y-6" x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Artikel Berita</h1>
                <p class="text-xs text-slate-500 mt-0.5 truncate max-w-md">{{ $post->title }}</p>
            </div>
        </div>
    ...
    <!-- Main Form -->
    <form id="postForm" action="{{ route('admin.posts.update', encode_id($post->id)) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-xs">
                    ...
                    <textarea name="content"
                              id="content"
                              rows="15"
                              class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs summernote">{{ old('content', $post->content) }}</textarea>
                </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-4 shadow-xs">
                    ...
                    <div>
                        ...
                        <textarea name="meta_description"
                                  ...
                                  placeholder="Ringkasan artikel untuk hasil pencarian Google (max 160 karakter)">{{ old('meta_description', $post->meta_description) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-4 shadow-xs">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs rounded-xl p-2.5 focus:bg-white focus:ring-1 focus:ring-indigo-500">
                            <option value="published" {{ old('status', $post->status) == 'published' ? 'selected' : '' }}>Published (Publik)</option>
                            <option value="draft" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>Draft (Konsep)</option>
                            <option value="archived" {{ old('status', $post->status) == 'archived' ? 'selected' : '' }}>Archived (Arsip)</option>
                        </select>
                    </div>
